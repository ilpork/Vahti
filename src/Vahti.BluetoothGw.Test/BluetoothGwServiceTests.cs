...
namespace Vahti.BluetoothGw.Test
{
    /// <Summary>
    /// Unit tests for <see cref="BluetoothGwService"/>
    /// </Summary>
    [TestClass]
    public class BluetoothGwServiceTests
    {
        private Mock<ILogger<BluetoothGwService>> _loggerMock;
        private Mock<IOptions<BluetoothGwConfiguration>> _configMock;
        ...
        {
            _loggerMock = new Mock<ILogger<BluetoothGwService>>();
            _configMock = new Mock<IOptions<BluetoothGwConfiguration>>();
            _deviceScannerMock = new Mock<IDeviceScanner>();
            _mqttClientMock = new Mock<IManagedMqttClient>();

            var services = new ServiceCollection();
            services.AddSingleton<BluetoothGwService>();
        ...
        public async Task ExecuteAsync_PublishMeasurementData_CorrectlyPublished()
        {
            ...
            //Arrange
            var config = new BluetoothGwConfiguration
            {
            ...
            //Act      
            var btGwService = _serviceProvider.GetService<BluetoothGwService>();
...
#pragma warning restore
            ...
            _deviceScannerMock.Verify(d => d.ScanDevicesAsync(It.IsAny<string>(), It.IsAny<int>()), 
                Times.Exactly(BluetoothGwService.MaxRepeatedReadFailCount), "The service did not stop after specified amount of failed reads");            
        }
